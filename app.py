...
st.markdown(
    ...
    "Upload your inverter Excel file or use a Google Sheet link and get detailed hourly & daily insights.")
...
with st.expander("📊 Data Source", expanded=False):
    ...
    "Choose Data Source:", [
        ...
        "🔗 Google Sheet Link", "📁 Upload Excel File"], horizontal=True, index=0)
    ...
    "Use different Google Sheet", value=False)
        ...
        try:
            ...
            st.info(
                ...
                "Make sure the sheet is published to web and you have the correct URL.")
...
if df is not None:
    ...
    col for col in df.columns if "time" in col or "date" in col][0]
    ...
    col for col in df.columns if "mode" in col or "status" in col][0]
    ...
    def calculate_daily_energy(df, datetime_col, calc_method):
        ...
        if calc_method == "Fixed 5 Minutes":
            ...
            st.sidebar.write(
                ...
                f"**Debug:** Each row = 5 minutes = {time_per_row_hours:.4f} hours")
            ...
            if len(time_diffs) > 0:
                ...
                st.sidebar.write(
                    ...
                    f"**Debug:** Auto-detected avg interval = {avg_minutes:.2f} min = {time_per_row_hours:.4f} hours")
                ...
                st.sidebar.write(
                    ...
                    f"**Debug:** Could not detect, using fallback = 5 min = {time_per_row_hours:.4f} hours")

        # Energy (kWh) = Power (W) × time_per_row_hours / 1000

        df_calc['solar_kwh'] = df_calc['pv_input_power_1'] * \
            time_per_row_hours / 1000

        df_calc['utility_kwh'] = df_calc['grid_power_input_active_total'] * \
            time_per_row_hours / 1000

        df_calc['load_kwh'] = df_calc['ac_output_active_power_total'] * \
            time_per_row_hours / 1000
        ...
        df_calc.loc[battery_condition,

    'battery_kwh'] = df_calc.loc[battery_condition,

    'ac_output_active_power_total'] * time_per_row_hours / 1000
        ...
        st.sidebar.write(
            ...
            f"**Solar kWh ({calc_method}):** {total_solar_kwh:.2f} kWh")

        st.sidebar.write(

            f"**Calculation:** {total_solar_power} × {time_per_row_hours:.4f} / 1000 = {total_solar_kwh:.2f} kWh")
        ...
        record_counts = df_calc.groupby(

            'date').size().reset_index(name='total_records')
    ...
    if len(selected_day_data) > 0:
        ...
        # But load_kwh includes battery portion, so we need to show breakdown
        ...
        grid_portion_load = selected_day['load_kwh'] - \
            selected_day['battery_kwh']
        ...
        total_all = selected_day['solar_kwh'] + \
            selected_day['utility_kwh'] + selected_day['battery_kwh']
    ...
    daily_energy_sorted = daily_energy_display.sort_values(
    ...
    # Build custom hover template with bar name and value first, then all 4
    ...
    load_view_mode = st.radio(

    "Load View Mode:", [

        "Hourly Average", "Row-wise (Every Entry)"], horizontal=True, index=1)

    # Sort data by datetime for row-wise view

    day_df_sorted_load = day_df.sort_values(

        datetime_col).reset_index(drop=True)
    ...
    numeric_cols_for_load = day_df_sorted_load.select_dtypes(
    ...
    title="Hourly Load Output % Wise (Average)")
    ...
    title="Load Output % Wise - Every Entry (Row-wise)")
        ...
        fig_load.update_traces(
            ...
            if any(p.replace('_', '') in col_lower.replace('_', '')
            ...
            datetime_col).reset_index(drop=True).copy()
            ...
            if col.lower() != voltage_col.lower() and col not in [

            datetime_col, 'date', 'hour']:
            ...
            # Build custom hover with key params - battery voltage value first,
            ...
            if col_lower != battery_col_lower and col not in [
            ...
            hovertemplate=battery_hover,

            customdata=battery_customdata)
            ...
            if col_lower != main_col_lower and col not in [
            ...
            f"**Auto-detected interval:** {avg_minutes:.2f} min per row")
            ...
            f"**Actual time span:** {int(actual_time_span_hours)}h {int((actual_time_span_hours % 1) * 60)}min")

            st.sidebar.write(f"**Records in day:** {total_records}")

            st.sidebar.write(

            f"**Grid:** {len(grid_records)} records → {int(grid_time_hours)}h {int((grid_time_hours % 1) * 60)}min")

            st.sidebar.write(

            f"**Solar:** {len(solar_records)} records → {int(solar_time_hours)}h {int((solar_time_hours % 1) * 60)}min")

            st.sidebar.write(

            f"**Battery:** {len(battery_records)} records → {int(battery_time_hours)}h {int((battery_time_hours % 1) * 60)}min")

            st.sidebar.write(

            f"**Mode Total:** {int(actual_mode_total)}h {int((actual_mode_total % 1) * 60)}min")
            ...
            f"{int(solar_time_hours)}h {int((solar_time_hours % 1) * 60)}m")

            col2.metric(

            "⚡ Grid Time",

            f"{int(grid_time_hours)}h {int((grid_time_hours % 1) * 60)}m")

            col3.metric(

            "🔋 Battery Time",

            f"{int(battery_time_hours)}h {int((battery_time_hours % 1) * 60)}m")

            # ===== BATTERY CHARGING ANALYSIS - Solar vs Grid =====

            st.subheader("🔋 Battery Charging Analysis - Voltages Used for Charging")

            day_df_charge = day_df.sort_values(
            ...
            day_df_charge['charging_source'] = day_df_charge.apply(

            classify_charging_source, axis=1)

            solar_charge_records = day_df_charge[day_df_charge['charging_source']

            == 'solar_charging']

            grid_charge_records = day_df_charge[day_df_charge['charging_source']

            == 'grid_charging']

            other_charge_records = day_df_charge[day_df_charge['charging_source']
            ...
            "☀️ Solar Charging Time",

            f"{int(solar_charge_time)}h {int((solar_charge_time % 1) * 60)}m")

            col_charge2.metric(

            "⚡ Grid Charging Time",

            f"{int(grid_charge_time)}h {int((grid_charge_time % 1) * 60)}m")

            total_charge_records = len(

            solar_charge_records) + len(grid_charge_records) + len(other_charge_records)

            col_charge3.metric(

            "📊 Total Charging Records",

            f"{total_charge_records}")
...
st.plotly_chart(fig_charge_mode, use_container_width=True, config={
    ...
    'doubleClick': False
        ...
        if len(solar_charge_records) > 0 or len(grid_charge_records) > 0:
            ...
            if len(solar_charge_records) > 0:
                ...
                solar_charge_kwh = solar_charge_records['pv_input_power_1'].sum(
            ...
            if len(grid_charge_records) > 0:
                ...
                grid_charge_kwh = grid_charge_records['grid_power_input_active_total'].sum(
    ...
    "☀️ Solar Energy to Battery",

    f"{solar_charge_kwh:.2f} kWh")

            col_en2.metric(

    "⚡ Grid Energy to Battery",

    f"{grid_charge_kwh:.2f} kWh")
            ...
            fig_charge_energy.update_traces(
                ...
                day_df_dual['power_source'] = day_df_dual.apply(

                classify_power_source, axis=1)

                dual_records = day_df_dual[day_df_dual['power_source'] == 'solar_grid']

                solar_only_records = day_df_dual[day_df_dual['power_source']

                == 'solar_only']

                grid_only_records = day_df_dual[day_df_dual['power_source'] == 'grid_only']

                battery_only_records = day_df_dual[day_df_dual['power_source']
                ...
                dual_records['ac_output_active_power_total'].sum() *

                time_per_row_hours /

                1000) if len(dual_records) > 0 else 0
                ...
                1000) if len(solar_only_records) > 0 else 0
                ...
                1000) if len(grid_only_records) > 0 else 0

                battery_only_load_kwh = (

                battery_only_records['ac_output_active_power_total'].sum() *

                time_per_row_hours /

                1000) if len(battery_only_records) > 0 else 0

                solar_contribution_kwh = (

                dual_records['pv_input_power_1'].sum() *
                ...
                dual_records['grid_power_input_active_total'].sum() *
                ...
                dual_sorted = dual_records.sort_values(
                ...
                current_time - prev_time).total_seconds() / 60

                if time_diff_minutes > (time_per_row_hours * 60 * 1.5):

                end_time = dual_sorted.iloc[i - 1][datetime_col]

                period_records = dual_sorted[(dual_sorted[datetime_col] >= start_time) & (

                dual_sorted[datetime_col] <= end_time)]

                period_load_kwh = period_records['ac_output_active_power_total'].sum(

                    ) * time_per_row_hours / 1000

                    period_solar_kwh = period_records['pv_input_power_1'].sum(

                    ) * time_per_row_hours / 1000

                    period_grid_kwh = period_records['grid_power_input_active_total'].sum(
    ...
    end_time - start_time).total_seconds() / 3600
        ...
        if dual_periods and len(dual_periods) > 0:
            ...
            for i, p in enumerate(dual_periods):
                ...
                period_charge = day_df_charge[(day_df_charge[datetime_col] >= period_start) & (

                    day_df_charge[datetime_col] <= period_end)]

                solar_charge_in_period = period_charge[period_charge['charging_source']

                    == 'solar_charging']

                grid_charge_in_period = period_charge[period_charge['charging_source']

                    == 'grid_charging']

                solar_excess = solar_charge_in_period['pv_input_power_1'].sum(

                ) * time_per_row_hours / 1000 if len(solar_charge_in_period) > 0 else max(0, solar_gen - load)

                grid_to_battery = grid_charge_in_period['grid_power_input_active_total'].sum(

                ) * time_per_row_hours / 1000 if len(grid_charge_in_period) > 0 else max(0, grid_draw - grid_to_load)
...
st.plotly_chart(fig_battery_charge, use_container_width=True, config={
    ...
    'doubleClick': False
            ...
            with st.expander("🔋 Battery Charging Timeline"):
                ...
                all_charge_records = day_df_charge[day_df_charge['charging_source'].isin(

                    ['solar_charging', 'grid_charging', 'other_charging'])]

                if len(all_charge_records) > 0:

                    st.write(

                        f"**Total Charging Records:** {len(all_charge_records)}")

                    st.write(

                        f"**Solar Charging:** {len(solar_charge_records)} records")

                    st.write(

                        f"**Grid Charging:** {len(grid_charge_records)} records")

                    charge_timeline = all_charge_records[[

                        datetime_col, battery_voltage_col, 'charging_source']].copy()

                    charge_timeline['Time'] = charge_timeline[datetime_col].dt.strftime(

                        '%H:%M')

                    st.dataframe(charge_timeline[[

                                'Time', battery_voltage_col, 'charging_source']], use_container_width=True)
...
st.caption(
    ...
    f"✓ Verify: {solar_to_load:.2f} + {grid_to_load:.2f} + {battery_to_load:.2f} = {verify:.2f}")
