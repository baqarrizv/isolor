...
components.html(
        ...
        (function(){
            ...
            function createToolbarFor(plotWrapper){
                if(plotWrapper._hasToolbar) return;
                plotWrapper._hasToolbar = true;
                plotWrapper.style.position = plotWrapper.style.position || 'relative';
                ...
                function setDrag(on){
                    ...
                    try{ Plotly.relayout(gd, {'dragmode': on ? 'zoom' : false}); }catch(e){console.warn(e)}
                }
                ...
                function zoom(factor){
                    ...
                    if(!gd || typeof Plotly === 'undefined') return;
                    try{
                        var xl = gd._fullLayout && gd._fullLayout.xaxis && gd._fullLayout.xaxis.range;
                        var yl = gd._fullLayout && gd._fullLayout.yaxis && gd._fullLayout.yaxis.range;
                        if(xl){
                            var cx = (xl[0]+xl[1])/2; var span = (xl[1]-xl[0])*factor/2;
                            var nx = [cx - span, cx + span];
                        } else { var nx = null; }
                        if(yl){
                            var cy = (yl[0]+yl[1])/2; var yspan = (yl[1]-yl[0])*factor/2;
                            var ny = [cy - yspan, cy + yspan];
                        } else { var ny = null; }
                        var obj = {};
                        if(nx) obj['xaxis.range'] = nx;
                        if(ny) obj['yaxis.range'] = ny;
                        if(Object.keys(obj).length) Plotly.relayout(gd, obj);
                    } catch(e){ console.warn(e); }
                }

                function reset(){
                    var gd = findGd(); if(!gd || typeof Plotly === 'undefined') return;
                    try{ Plotly.relayout(gd, {'xaxis.autorange': true, 'yaxis.autorange': true}); }catch(e){console.warn(e)}
                }
                ...
                sbtn.addEventListener('click', function(){ safe(save); });
            ...
            function scan(){
                try{
                    var pdoc = window.parent.document;
                    var plots = pdoc.querySelectorAll('[data-testid="stPlotlyChart"]');
                    plots.forEach(function(w){ createToolbarFor(w); });
...
if df is not None:
    ...
    with col_hdr_mid:
        st.header(f"📅 {date_options[st.session_state['date_index']]} — {st.session_state['date_index'] + 1}/{len(date_options)}")
    with col_hdr_right:
    ...
    if len(selected_day_data) > 0:
        ...
        # Make chart responsive for mobile (show Plotly modebar)
    ...
    fig_voltage.update_layout(hovermode='closest', hoverdistance=-1, dragmode=False)
    ...
    if battery_col:
        ...
        fig_battery.update_layout(hovermode='closest', hoverdistance=-1, dragmode=False)
    ...
    fig_main.update_layout(hovermode='closest', hoverdistance=-1, dragmode=False)
