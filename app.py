...
if data_source == "dY"- Google Sheet Link":
    ...
    try:
        ...
        st.success("Google Sheet Loaded Successfully ƒo.")
        
    except Exception as e:
        st.error(f"ƒsÿ‹,? Error loading Google Sheet: {str(e)}")
    ...
    if uploaded_file is not None:
        try:
            ...
            st.success(f"Loaded uploaded file: {uploaded_file.name} ƒo.")
        except Exception as e:
        ...
        if os.path.exists(local_file):
            try:
                ...
                st.success(f"Loaded local file: {local_file} ƒo.")
            except Exception as e:
...
if df is not None:
    ...
    try:
        ...
        st.error("ƒsÿ‹,? Could not detect required columns. Please ensure your Excel file has columns containing: time/date, load, voltage, mode/status.")
    ...
    if df[datetime_col].isna().all():
        st.error("ƒsÿ‹,? Could not parse datetime column. Please check your data format.")
        st.stop()

    df["date"] = df[datetime_col].dt.date
    df["hour"] = df[datetime_col].dt.hour

    st.success("File Loaded Successfully ƒo.")
    ...
    if len(date_options) == 0:
        st.error("ƒsÿ‹,? No valid dates found in the data.")
        st.stop()
    ...
    if len(selected_day_data) > 0:
        ...
        col_a.metric("ƒ~?‹,? Solar se", f"{selected_day['solar_kwh']:.2f} units")
        col_b.metric("ƒs­ Grid se", f"{selected_day['utility_kwh']:.2f} units")
        col_c.metric("dY?ÿ Total Load", f"{selected_day['load_kwh']:.2f} units")
    ...
    st.subheader("ƒs­ Inverter Operation Mode Time Calculation")
    ...
    col1, col2 = st.columns(2)
    col1.metric("Full Battery (ƒ%^100%)", f"{len(full_battery)} records - (V) ƒ% 28.5V")
    
    # Low battery indicator
    low_battery = day_df[(day_df[voltage_col] < 24.0)]
    col2.metric("Low Battery (ƒ%^0-20%)", f"{len(low_battery)} records - (V) < 24V")
    ...
    if performance_score >= 70:
        st.success("ƒo. Great performance! Inverter is working efficiently.")
    elif performance_score >= 40:
        st.warning("ƒsÿ‹,? Average performance. Check battery charging.")
    else:
        st.error("ƒ?O Poor performance. Needs attention!")
